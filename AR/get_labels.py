...
def get_labels(frame, bboxes):
    ...
    **为每个 `bbox` 分配颜色标签，确保仅有一个 person1 和一个 person2，并保持 bbox 原始顺序**
    ...
    for bbox in bboxes:
        ...
        yellow_mask = get_color_mask(person_roi, yellow_lower, yellow_upper)

        red_ratio = get_color_ratio(red_mask)
        green_ratio = get_color_ratio(green_mask)
        yellow_ratio = get_color_ratio(yellow_mask)

        color_ratios.append((red_ratio, green_ratio, yellow_ratio, bbox))
    ...
    if len(color_ratios) == 1:
        red_ratio, green_ratio, yellow_ratio, bbox = color_ratios[0]
        if red_ratio > green_ratio and red_ratio > yellow_ratio:
            results_dict[tuple(bbox)] = ('person1', red_ratio)
        elif green_ratio > red_ratio and green_ratio > yellow_ratio:
            results_dict[tuple(bbox)] = ('person2', green_ratio)
        else:
            results_dict[tuple(bbox)] = ('person01', yellow_ratio)

        # Two people, ensure one person1 and one person2
    elif len(color_ratios) == 2:
        (r1, g1, y1, bbox1), (r2, g2, y2, bbox2) = color_ratios

        # If one is clearly red and one is clearly green, direct assignment
        if r1 > g1 and r1 > y1 and g2 > r2 and g2 > y2:
            results_dict[tuple(bbox1)] = ('person1', r1)
            results_dict[tuple(bbox2)] = ('person2', g2)
        elif g1 > r1 and g1 > y1 and r2 > g2 and r2 > y2:
            results_dict[tuple(bbox1)] = ('person2', g1)
            results_dict[tuple(bbox2)] = ('person1', r2)
        else:
            # Otherwise, assign based on red-green difference
            if r1 - g1 > r2 - g2:
                results_dict[tuple(bbox1)] = ('person1', r1)
                results_dict[tuple(bbox2)] = ('person2', g2)
            else:
    ...
    elif len(color_ratios) >= 3:
        # Sort by red ratio descending, then by green ratio descending
        sorted_people = sorted(color_ratios, key=lambda x: (x[0], x[1]), reverse=True)

        # Assign person1 to the one with highest red ratio
        results_dict[tuple(sorted_people[0][3])] = ('person1', sorted_people[0][0])

        # Assign person2 to the one with highest green ratio among the rest
        remaining = sorted(color_ratios[1:], key=lambda x: x[1], reverse=True)
        results_dict[tuple(remaining[0][3])] = ('person2', remaining[0][1])

        # Assign everyone else as person3
        for _, _, yellow_ratio, bbox in color_ratios:
            if tuple(bbox) not in results_dict:
                results_dict[tuple(bbox)] = ('person01', yellow_ratio)

        # Ensure results are returned in original bbox order
    results = [results_dict[tuple(bbox)] for bbox in bboxes]
